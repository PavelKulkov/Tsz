...
class Forms {
	...
	public function saveResponse(&$data,$idRequest,$code,$id_out,$currentDate, $comment = '', $user_action = ''){
		$this->db_instance->changeDB("regportal_share");
		$time = $currentDate->format("Y-m-d H:i:s");
		$hash = md5($data);
		$this->db_instance->insert('INSERT INTO `response` (`id_request`, `time`, `state`, `id_out`, `hash`, `comment`, `user_action`) VALUES (?, ?, ?, ?, ?, ?, ?)', $idRequest, $time , $code, $id_out, $hash, $comment, $user_action);
